...
public interface FestivalRepo extends JpaRepository<Festival, Long> {
    ...
    @Query(value = "SELECT * FROM festival WHERE festival_name ILIKE %:name% ORDER BY festival_date,festival_name", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM festival WHERE festival_location ILIKE %:location% ORDER BY festival_date, festival_name", nativeQuery = true)
    ...
    @Query(value = "SELECT DISTINCT f.* FROM festival f " +
            ...
            "WHERE a.artist_name ILIKE %:artist% ORDER BY festival_date, festival_name", nativeQuery = true)
